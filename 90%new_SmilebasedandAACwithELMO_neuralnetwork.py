...
#        self.fc4 = nn.Linear(1024, 2150)
        self.fc4 = nn.Linear(3112,100)#2048)
...
#            if loss<0.58:#70%
#                break;
            if loss<0.45:#0.45#new#0.55:#90%#0.74 #0.78 average for external
                break;
#            print("Loss=",loss)
            if ( epoch) % 50== 0:
...
#Clinical_data_result(Classifier,FeatureName)
####External testing
#path='D:/PhD/Hemo_All_SeQ/'
...
#External_score =Hemonet(External_features).cpu().data.numpy()
...
ALL_DRAMP_Clinical_data_score =Hemonet(ALL_DRAMP_Clinical_data_Features).cpu().data.numpy()
...
#plt.show()
#plt.figure()
#plt.hist(ALL_DRAMP_Clinical_data_score,bins=len(ALL_DRAMP_Clinical_data_score),color='m')
#sorted_ALL_DRAMP_Clinical_data_score=np.sort(ALL_DRAMP_Clinical_data_score)
plt.plot(np.histogram(ALL_DRAMP_Clinical_data_score,bins=14)[0],color='m',marker='o',label='Our method:{: .2f}'.format(np.mean(ALL_DRAMP_Clinical_data_score)))
#plt.hist(np.sort(ALL_DRAMP_Clinical_data_score),bins=len(ALL_DRAMP_Clinical_data_score),color='r')
#plt.grid()
