...
import torch
from torch.autograd import Variable
from torch.nn import Linear, ReLU, CrossEntropyLoss, Sequential, Conv2d, MaxPool1d, Module, Softmax, BatchNorm2d, Dropout
...
def define_model():
	model = Sequential()
	model.add(Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(3118,1)))
...
#            BatchNorm1d(3),
            ReLU(inplace=True),
            MaxPool1d(kernel_size=1, stride=2),
...
#np.save(path+"3112_F.npy",Features)
